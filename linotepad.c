...
static void text_snapshot_free(gpointer data) {
    TextSnapshot *snapshot = data;
    if (!snapshot) return;
    g_free(snapshot->text);
    g_free(snapshot);
}

static TextSnapshot *text_snapshot_new(AppState *app) {
    GtkTextIter start, end, cursor;
    TextSnapshot *snapshot = g_new0(TextSnapshot, 1);

    gtk_text_buffer_get_bounds(app->buffer, &start, &end);
    snapshot->text = gtk_text_buffer_get_text(app->buffer, &start, &end, FALSE);
    gtk_text_buffer_get_iter_at_mark(app->buffer, &cursor,
        gtk_text_buffer_get_insert(app->buffer));
    snapshot->cursor_offset = gtk_text_iter_get_offset(&cursor);
    return snapshot;
}

static void clear_history_stack(GList **stack) {
    g_list_free_full(*stack, text_snapshot_free);
    *stack = NULL;
}
...
static void trim_history_stack(GList **stack) {
    ...
    text_snapshot_free(last->data);
    *stack = g_list_delete_link(*stack, last);
}

static void remember_undo_state(AppState *app) {
    TextSnapshot *snapshot = text_snapshot_new(app);
    TextSnapshot *top = app->undo_stack ? app->undo_stack->data : NULL;

    if (top && g_strcmp0(top->text, snapshot->text) == 0) {
        text_snapshot_free(snapshot);
        return;
    }

    app->undo_stack = g_list_prepend(app->undo_stack, snapshot);
...
static void reset_history(AppState *app) {
    ...
    remember_undo_state(app);
}

static void apply_text_snapshot(AppState *app, TextSnapshot *snapshot) {
    GtkTextIter cursor;
    gint chars;

    app->applying_history = TRUE;
    gtk_text_buffer_set_text(app->buffer, snapshot->text, -1);
    chars = gtk_text_buffer_get_char_count(app->buffer);
    gtk_text_buffer_get_iter_at_offset(app->buffer, &cursor,
        MIN(snapshot->cursor_offset, chars));
    gtk_text_buffer_place_cursor(app->buffer, &cursor);
