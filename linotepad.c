...
static void update_title(AppState *app) {
    const gchar *base = app->filename ? g_path_get_basename(app->filename) : "Untitled";
    gchar *title = g_strdup_printf("%s%s - Linotepad",
                                    app->dirty ? "*" : "", base);
    gtk_window_set_title(GTK_WINDOW(app->window), title);
    g_free(title);
    if (app->filename) g_free((gchar*)base);
}
...
static void on_delete(GtkMenuItem *item, AppState *app) {
    (void)item;
    gtk_text_buffer_delete_selection(app->buffer, TRUE, TRUE);
}
...
int main(int argc, char **argv) {
    ...
    add_item(file_menu, "_New", G_CALLBACK(on_new), app, "<Control>N", accel);
    ...
    if (argc > 1) {
        set_buffer_text_from_file(app, argv[1]);
    } else {
        update_title(app);
    }
